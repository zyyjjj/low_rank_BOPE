...
sys.path.append('/home/yz685/')
sys.path.append('/home/yz685/low_rank_BOPE/experiments/')
...
# problem-specific params -- read from yaml file
...
def run_one_trial(
    ...
    config: Dict[str, Any],
    verbose: bool,
    save_path_partial: str
    # **tkwargs,
) -> Dict:

    problem = LunarLander(
            num_envs=config['outcome_dim'],
            min_reward=config['baseline_reward']
        )
    util_func = Sigmoid(config["sigmoid_coeff"], config["min_reward_diff"])
            ...
            "outcome_tf": ChainedOutcomeTransform(
                **{
                    "standardize": Standardize(
                        config["outcome_dim"],
                        min_stdv=config["standardize_min_std"], 
                    ),
                    # "pca": PCAOutcomeTransform(num_axes=config["lin_proj_latent_dim"]),
                    "pca": PCAOutcomeTransform(
                        variance_explained_threshold=config["pca_var_threshold"]
                    ),
                }
            ),
        },
        ...
        for pe_strategy in ["EUBO-zeta", "Random-f"]:
            ...
            if fit_pref_model_succeed:
                outcome_model_mse = check_outcome_model_fit(
                    outcome_model=outcome_model, problem=problem, n_test=config['n_test_check_model_fit'],
                    batch_eval = False
                )
                print(f"outcome model mse: {outcome_model_mse}")
                pref_model_acc = check_pref_model_fit(
                    pref_model, problem=problem, util_func=util_func, n_test=config['n_test_check_model_fit'],
                    batch_eval = False
                )
    ...
    save_path = save_path_partial + 'outcome_dim=' + str(config['outcome_dim']) + \
        '_trial=' + str(trial_idx) + '.pt'
...
run_one_trial(
    ...
    config = config, 
    verbose = True,
    save_path_partial = '/home/yz685/low_rank_BOPE/experiments/results/lunar_lander/'
)
